...
def _check_reminders_secret(provided: Optional[str]) -> None:
    """Autorise via REPORT_SECRET ou REMINDERS_TRIGGER_TOKEN."""
    candidates = [
        (os.getenv("REPORT_SECRET") or "").strip(),
        (os.getenv("REMINDERS_TRIGGER_TOKEN") or "").strip(),
    ]
    valid = [c for c in candidates if c]
    if not valid:
        raise HTTPException(status_code=503, detail="Reminders trigger not configured")
    if (provided or "").strip() not in valid:
        raise HTTPException(status_code=403, detail="Invalid secret")
