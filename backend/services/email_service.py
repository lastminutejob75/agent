...
def send_patient_questionnaire_email(
    ...
    """Envoie au patient le lien sécurisé pour remplir son questionnaire médical."""
    to_addr = (to_email or "").strip().lower()
    if not to_addr:
        return False, "Destinataire vide"
    ...
    subject = f"{cabinet} — Questionnaire médical à compléter"
    html = f"""
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Questionnaire médical</title></head>
<body style="margin:0;background:#0a1628;padding:40px 20px;font-family:'DM Sans',Arial,sans-serif;">
  <div style="max-width:520px;margin:0 auto;">
    <div style="font-size:24px;font-weight:800;color:#00d4a0;margin-bottom:8px">UWI</div>
    <h1 style="color:#ffffff;font-size:22px;line-height:1.3;margin:0 0 12px 0">
    ...
    <p style="color:rgba(255,255,255,0.72);font-size:15px;line-height:1.6;margin:0 0 28px 0">
      {cabinet} vous invite à compléter votre questionnaire médical avant votre rendez-vous.
      Cela prend moins de 3 minutes et permet de mieux vous accueillir.
    </p>
    <p style="margin:0 0 20px 0;">
...
"""
    ...
    if token and from_addr:
        try:
            ok, err = _send_via_postmark(from_addr, to_addr, subject, html, token)
            if ok:
                logger.info("patient_questionnaire_email_sent via postmark", extra={"to": to_addr[:50]})
            return ok, err
        except Exception as e:
            logger.exception("send_patient_questionnaire_email postmark failed")
            return False, str(e)
    smtp_user = (os.getenv("SMTP_EMAIL") or "").strip()
    smtp_pass = (os.getenv("SMTP_PASSWORD") or "").strip()
    if smtp_user and smtp_pass:
        ...
        try:
            ...
            with smtplib.SMTP(host, port, timeout=15) as server:
                ...
                server.login(smtp_user, smtp_pass)
                server.sendmail(smtp_user, [to_addr], msg.as_string())
            logger.info("patient_questionnaire_email_sent via smtp", extra={"to": to_addr[:50]})
            return True, None
        except Exception as e:
            logger.exception("send_patient_questionnaire_email smtp failed")
            return False, str(e)
    return False, "Email non configuré (Postmark ou SMTP)"
