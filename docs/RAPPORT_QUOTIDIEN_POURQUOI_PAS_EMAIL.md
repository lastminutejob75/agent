...
## Exemple concret (Lastminutejob)

- **URL backend** : `https://agent-production-c246.up.railway.app`
- **Email qui doit recevoir le rapport** : sur Railway, définir `REPORT_EMAIL=henigoutal@gmail.com` (ou `OWNER_EMAIL`).

Test rapide (remplacer `TON_SECRET` par la valeur de `REPORT_SECRET` définie sur Railway) :

```bash
curl -s -X POST "https://agent-production-c246.up.railway.app/api/reports/daily" \
  -H "X-Report-Secret: TON_SECRET"
```

Interprétation : **503** → ajouter `REPORT_SECRET` sur Railway ; **403** → secret différent entre GitHub et Railway ; **200** + `email_skipped` → corriger la config indiquée ; **200** + `clients_notified: 1` sans `email_skipped` → email envoyé (vérifier les spams).
...
| **200** + `"email_error": "..."` | SMTP configuré mais envoi échoué (auth, connexion…). Le message indique la cause. |

---

## 2. Checklist de vérification (Railway)

Cocher que chaque variable est bien définie :

- [ ] **REPORT_EMAIL** (ou OWNER_EMAIL) = ton adresse qui reçoit (ex. `henigoutal@gmail.com`)
- [ ] **REPORT_SECRET** = même valeur que sur GitHub Actions
- [ ] **SMTP_EMAIL** = adresse Gmail qui envoie
- [ ] **SMTP_PASSWORD** = mot de passe d'application Gmail (Sécurité → Mots de passe d'application), pas le mot de passe du compte
- [ ] **SMTP_HOST** = `smtp.gmail.com`
- [ ] **SMTP_PORT** = `587`

---

## 3. Variables à avoir sur Railway
...
## 4. Vérifier que le job GitHub tourne à 19h Paris
...
## 5. Résumé des causes « je ne reçois pas l'email »
