...
function PatientProfileTextArea({
  ...
  return (
    <label className={cx("rounded-2xl bg-white p-3", className)}>
      ...
      <textarea
        ...
        className="w-full resize-y rounded-xl border border-[#DDE7F1] bg-white px-3 py-2 text-sm font-semibold leading-5 text-[#0A1628] outline-none placeholder:text-[#94A3B8] focus:border-[#009CA4]"
      />
    </label>
  );
}
...
export default function PatientDashboardPage() {
  ...
  const consultationPatient = useMemo(() => {
    ...
    return {
      ...
      points_attention: readProfileField(["points_attention", "warning_points"]),
  ...
  const submitConsultationForm = useCallback(async (draft: Record<string, unknown>) => {
    ...
    try {
      ...
      try {
        ...
        // Best effort: la fiche a bien été enregistrée, le prochain chargement récupérera le contexte patient enrichi.
      }
